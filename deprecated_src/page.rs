...
// global_asm!(include_str!("./arch/riscv/rv64gc/mem.S"));
// extern "C" {
...
//         for i in 0..size {
//             // We use big_ptr so that we can force an
//             // sd (store doubleword) instruction rather than the
//             // sb (store byte). This means 9x fewer stores than before.
//             // Typically we have to be concerned about remaining
//             // bytes, but fortunately 4096 % 8 = 0, so we
//             // won't have any remaining bytes.
//             unsafe {
...
//         let addr = HEAP_START + (ptr as usize - ALLOC_START) / PAGE_SIZE;
//         // Make sure that the address makes sense. The address we
//         // calculate here is the page stucture, not the HEAP address.
//         assert!(addr >= HEAP_START && addr < HEAP_START + HEAP_SIZE);
//         let mut p = addr as *mut Page;
//         // Keep clearing pages until we hit the last page.
//         while (*p).is_taken() && !(*p).is_last() {
...
//         // If the following assertions fails, it is most likely
//         // caused by a double-free.
//         assert!(
//             (*p).is_last() == true,
//             "Possible double-free detected! (Not taken found before last)"
//         );
//         // If we get there, we've taken care of all previous pages and
//         // we are on the last page.
//         (*p).clear();
//     }
// }
...
// /// Print all page allocations
// /// This is mainly used for debugging.
// pub(crate) fn print_page_allocations() {
//     use crate::{print, println};
//     unsafe {
//         let num_pages = HEAP_SIZE / PAGE_SIZE;
//         let mut beg = HEAP_START as *const Page;
...
// // A single bit, We're using an i64 so that
// // this will sign-extend rather than zero-extend
// // since RISC-V requires that the reserved sections
// // take on the most significant bit.
// pub struct Entry {
...
//     pub fn is_valid(&self) -> bool {
//         self.get_entry() & EntryBits::Valid.as_i64() != 0
//     }
...
//     // otherwise, we'll leak memory and always create a page fault.
//     assert!(bits & 0xe != 0);
//     // Extracte out each VPN from the virtual address
//     // One the virtual address, each VPN is exactly 9 bits,
//     // which is why we use the mask 0x1ff = 0b1_1111_1111 (9 bits)
//     let vpn = [
//         // VPN[0] = vaddr[20:12]
//         (vaddr >> 12) & 0x1ff,
...
//             // Allocate a page
//             let page = zalloc(1);
//             // The page is alread aligned by 4096, so store it
//             // directly. The page is stored in the entry shifted
//             // right by 2 places.
//             v.set_entry((page as i64 >> 2) | EntryBits::Valid.as_i64());
//         }
...
// /// The reason wedon't free the root is because it is
// /// usually embedded into the Process structure.
// pub fn unmap(root: &mut Table) {
//     // Start with level 2
//     for lvl2 in 0..Table::len() {
//         let ref entry_lvl2 = root.entries[lvl2];
//         if entry_lvl2.is_valid() && entry_lvl2.is_branch() {
//             // This is a valid entry, so drill down and free.
//             let memaddr_lvl1 = (entry_lvl2.get_entry() & !0x3ff) << 2;
//             let table_lvl1 = unsafe { (memaddr_lvl1 as *mut Table).as_mut().unwrap() };
//             for lvl1 in 0..Table::len() {
//                 let ref entry_lvl1 = table_lvl1.entries[lvl1];
//                 if entry_lvl1.is_valid() && entry_lvl1.is_branch() {
//                     let memaddr_lvl0 = (entry_lvl1.get_entry() & !0x3ff) << 2;
//                     // The next level is level 0, which
//                     // cannot have branches therefore, we free here.
//                     dealloc(memaddr_lvl0 as *mut u8);
//                 }
//             }
//             dealloc(memaddr_lvl1 as *mut u8);
//         }
//     }
// }
