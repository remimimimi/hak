...
extern "C" fn kinit() -> usize {
    ...
    uart::UartDriver::new(0x1000_0000).init();
    page::init();
    kmem::init();
...
pub fn id_map_range(root: &mut page::Table, start: usize, end: usize, bits: i64) {
    ...
    let num_kb_pges = (page::align_val(end, 12) - memaddr) / page::PAGE_SIZE;
    // I named this num_kb_pges for future expansion when
    // I decide to allow for GiB (2^30) and 2MiB (2^21) page
    // sizes. However, the overlapping memory regions
    // are causing nightmares.
    for _ in 0..num_kb_pges {
